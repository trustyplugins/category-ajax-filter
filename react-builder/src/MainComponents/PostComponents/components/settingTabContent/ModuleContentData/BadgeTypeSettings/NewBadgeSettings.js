...
function NewBadgeSettings(props) {
  ...
  const [customText, setCustomText] = useState(
    String(newSettings?.custom_text ?? "").trim() || BADGE_DEFAULT_TEXT.new,
  );
  ...
  useEffect(() => {
    ...
    setCustomText(
      String(nextNewSettings?.custom_text ?? "").trim() ||
  ...
  const handleCustomTextBlur = (event) => {
    const normalized =
      String(event.target.value ?? "").trim() || BADGE_DEFAULT_TEXT.new;
