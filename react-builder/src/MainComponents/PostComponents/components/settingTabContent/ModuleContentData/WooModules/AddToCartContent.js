...
const resolveAfterAtcText = (value) =>
  normalizeFreeformLabelText(value, DEFAULT_AFTER_ATC_TEXT);

const syncButtonTextByTypeForUi = (raw) => {
...
function AddToCartContent(props) {
  ...
  const [afterAtcText, setAfterAtcText] = useState(
    getFreeformLabelTextForUi(
      modSettings?.after_atc_text,
      DEFAULT_AFTER_ATC_TEXT,
    ),
  );
  ...
  const handleProductTypeTextBlur = (e) => {
    const fallback =
      BUTTON_TEXT_BY_TYPE_DEFAULTS[productTypeKey] || DEFAULT_CUSTOM_BUTTON_TEXT;
    const normalized = normalizeFreeformLabelText(e.target.value, fallback);
    const nextByType = {
      ...buttonTextByType,
