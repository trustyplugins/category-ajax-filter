...
// console.log(item);
  ...
  // Star tab is Product Rating + stars only. Keep a render-time fallback so
  // switching modules while Star is selected cannot crash before useEffect runs.
  const isStarSubTabAvailable =
    type === "module" &&
    module?.key === "woo_product_rating" &&
    (item?.rating_display || "stars") === "stars";
  const safeSelectedSubTab =
    selectedSubTab === "star" && !isStarSubTabAvailable
      ? "container"
      : selectedSubTab;

  const effectiveLayoutStyleTab =
    safeSelectedSubTab !== "container" ? safeSelectedSubTab : "";
  ...
  let styleItems = [
    ...buildPostDesignTabStyleItemsPart1({
      ...
      item,
      modulesKeysArray,
      selectedSubTab: safeSelectedSubTab,
      handleSettingChange,
      SubTabLayoutItems,
      subTabCommonItems,
      textSubTabItems,
