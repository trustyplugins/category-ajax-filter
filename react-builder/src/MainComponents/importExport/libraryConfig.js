 ...
 * Free: local plugin import-library/ via WP AJAX.
 * Pro: remote trustyplugins.com library via WP AJAX.
 */
...
/**
 * Live demo URL for a library card, if the template ships one.
 *
 * @param {object} item Template row.
 * @returns {string}
 */
export const resolveLibraryDemoUrl = (item) => {
  if (!item || typeof item !== "object") {
    return "";
  }

  const raw = item.demoUrl || item.demo_url || "";
...
export const normalizeLibraryTemplate = (item) => {
  ...
  return {
    ...
    previewImage,
    previewCardLayout,
    demoUrl: resolveLibraryDemoUrl(item),
    tier,
