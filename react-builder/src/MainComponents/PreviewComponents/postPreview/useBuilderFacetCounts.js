...
const buildFacetCountsSignature = (mainBuilderData) => {
  ...
  return JSON.stringify({
    ...
    dynamicTermCounts: extraData.dynamic_term_counts ?? "false",
 ...
 * on stale layout-baked term counts. Uses the lightweight facet_counts
...
const useBuilderFacetCounts = (mainBuilderData, { enabled = true } = {}) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const query_data = {
        query: {
          post_type: resolvePostTypeFromBuilderData(builderData),
          post_status: "publish",
          posts_per_page: 1,
          paged: 1,
        },
