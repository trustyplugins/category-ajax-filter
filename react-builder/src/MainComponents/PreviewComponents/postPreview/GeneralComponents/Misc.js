...
import {
  shouldOfferWooCatalogOrderby,
  WOO_CATALOG_ORDERBY_OPTIONS,
              ...
              {dndColData?.[column_index]?.data?.[item_index]?.settings?.['order_by']?.is_enable === "true" && (
                <div className="module-content-tab-row post-order-by no-pad-0">
                  ...
                  {shouldOfferWooCatalogOrderby(props.mainBuilderData) &&
                    WOO_CATALOG_ORDERBY_OPTIONS.map((option) => (
                      <div
                      ...
                      >
                        <Tooltip
                          ...
                          placement="topLeft"
                          title={`Allow sorting by ${option.label.toLowerCase()}.`}
                        >
                          <label>{option.label}</label>
                        </Tooltip>
                        <Switch
                          onChange={(checked) =>
                            onChangeOrderBy(checked, option.value)
                          }
                          checked={
                            Array.isArray(OrderByArr) &&
                            OrderByArr.includes(option.value)
                          }
                        />
                      </div>
                    ))}
