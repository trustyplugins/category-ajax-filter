...
import { getUpgradeUrl, isModuleLocked, canUseProductPostType, isProTier } from "../../../tier/capabilities";
...
const NewModulePopUp = (props) => {
  ...
  const layoutPostType = props.postType ? String(props.postType) : "";

  const getModuleLockTooltip = (item) => {
    if (item.key === "range_slider" && !isProTier()) {
      return (
        <span className="caf-builder-tier-locked-section__tooltip-text">
          On Free, Range Slider is available only for WooCommerce Product
          layouts. Upgrade to Pro to use Range Slider with custom fields on any
          post type.{" "}
          <a
            href={getUpgradeUrl()}
            target="_blank"
      ...
      >
        {moduleLocked ? (
          <Tooltip
            ...
            placement="topLeft"
            title={getModuleLockTooltip(item)}
          >
