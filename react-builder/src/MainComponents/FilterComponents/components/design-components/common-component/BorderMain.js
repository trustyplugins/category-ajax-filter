...
import { Input, Slider, ColorPicker, Select, Row, Col, Tooltip, InputNumber, Space } from "antd";
...
function BorderMain(props) {
  ...
  const { property, label, styleState = "default", styleTab: originalStyleTab, deviceSwitch, isMeta } = props;
  ...
  useEffect(() => {
    ...
    if (type === "row") {
      ...
      rowObjShallowCopy = { ...RowStyle[device] };
      item = { ...rowObjShallowCopy[styleState] };
    } else if (type === "column") {
      let ColStyle = props.data[rowindex].data[columnindex].style;
      colObjShallowCopy = { ...ColStyle[device] };
      item = { ...colObjShallowCopy[styleState] };
    } else if (type === "module") {
      let ModuleStyle = props.data[rowindex].data[columnindex].data[moduleindex].style;
      if (styleTab !== "") {
        objShallowCopy = { ...ModuleStyle[styleTab] };
        deviceCopy = { ...objShallowCopy[device] };
  ...
  const resetBorderRadius = () => {
    ...
    if (type == "module") {
      let ModuleStyle = props.data[rowindex].data[columnindex].data[moduleindex].style;
      if (styleTab != "") {
        let mSwcopy = { ...ModuleStyle[styleTab] }
        let deviceCopy = { ...mSwcopy[device] }
        item = { ...deviceCopy[styleState] }
        item["borderTopLeftRadius"] = 0 + "px";
        item["borderTopRightRadius"] = 0 + "px";
        item["borderBottomLeftRadius"] = 0 + "px";
      ...
      <div className="caf-builder-border-container">
        <div className="caf-builder-setting-row-label">
          <label>
            ...
            <Tooltip classNames={{ root: "caf-builder-tooltip" }} placement="topLeft" title="Reset">
              <span onClick={resetBorderRadius}><FontAwesomeIcon icon={faArrowRotateLeft} /></span>
            </Tooltip>
          </label>
        ...
        <div className="caf-builder-setting-row-label">
          <label>
            ...
            <Tooltip classNames={{ root: "caf-builder-tooltip" }} placement="topLeft" title="Reset">
              <span onClick={resetBorderStyle}><FontAwesomeIcon icon={faArrowRotateLeft} /></span>
            </Tooltip>
          </label>
        ...
        >
          <label>
            ...
            <Tooltip classNames={{ root: "caf-builder-tooltip" }} placement="topLeft" title="Reset">
              <span onClick={() => onChangeWidth(0, 'reset')}><FontAwesomeIcon icon={faArrowRotateLeft} /></span>
            </Tooltip>
          </label>
          <Row>
            <Col span={15}>
        ...
        >
          <label>
            ...
            <Tooltip classNames={{ root: "caf-builder-tooltip" }} placement="topLeft" title="Reset">
              <span onClick={() => setColorHexFun("#ffffff00", 'reset')}><FontAwesomeIcon icon={faArrowRotateLeft} /></span>
            </Tooltip>
          </label>
          <ColorPicker
        ...
        >
          <label>
            ...
            <Tooltip classNames={{ root: "caf-builder-tooltip" }} placement="topLeft" title="Reset">
              <span onClick={() => handleChange("solid", 'reset')}><FontAwesomeIcon icon={faArrowRotateLeft} /></span>
            </Tooltip>
          </label>
          <Select
            defaultValue={
