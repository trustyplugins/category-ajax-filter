...
/**
 * Checkbox/dropdown Design tab: icon/swatch styling sub-tabs
 * (Sizing, Spacing, Background, Border, Box Shadow).
 * Shown whenever Show Icon / Color Swatch is on.
 */
export const shouldShowFilterDesignIconStyleTab = (settings) => {
  return String(settings?.show_icon) === "true";
};

/**
 * Design → Single Item → Text only: hide Color Swatch sub-tab.
 * Term fill colors live in Content; Text typography does not apply to the swatch.
 */
export const shouldShowFilterDesignIconStyleTabUnderText = (settings, moduleKey) => {
  const key = String(moduleKey || "").trim();
  if (key !== "checkbox_filter" && key !== "dropdown_filter") {
    return shouldShowFilterDesignIconStyleTab(settings);
  }
