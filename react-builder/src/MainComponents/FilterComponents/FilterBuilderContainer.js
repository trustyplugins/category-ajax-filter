...
import {
  sanitizeImportedFilterModule,
  sanitizeImportedFilterModulesInTree,
} from "../importExport/sanitizeImportedFilterTerms";
import {
  ENTITY_TIER_IMPORT_BLOCKED_MESSAGE,
  sanitizeFilterEntityForTier,
} from "../../tier/sanitizeLayoutForTier";
import {
  layoutIndexEquals,
...
function FilterBuilderContainer(props) {
  ...
  const applyWidgetImport = async () => {
    ...
    const layoutPostType = resolvePostTypeFromBuilderData(props.mainBuilderData);
    const tierSafeModule = sanitizeFilterEntityForTier(nextModule, {
      postType: layoutPostType,
    });
    if (!tierSafeModule) {
      throw new Error(ENTITY_TIER_IMPORT_BLOCKED_MESSAGE);
    }
    layoutCopy[rowindex].data[columnindex].data.push(tierSafeModule);
    setInitialData([...layoutCopy]);
    ...
    setIndexes({
      ...
      module: tierSafeModule,
    });
    handleSettingPopUp("module", rowindex, columnindex, insertedIndex, tierSafeModule);
  };
  const applyColumnImport = async () => {
    ...
    const tierSafeColumn = sanitizeFilterEntityForTier(nextColumn, {
      postType: resolvePostTypeFromBuilderData(props.mainBuilderData),
    });
    if (!tierSafeColumn) {
      throw new Error(ENTITY_TIER_IMPORT_BLOCKED_MESSAGE);
    }
    layoutCopy[rowindex].data.splice(insertIndex, 0, tierSafeColumn);
    setInitialData([...layoutCopy]);
    setIndexes({
      ...indexes,
      type: "column",
  ...
  const applyRowImport = async () => {
    ...
    const tierSafeRow = sanitizeFilterEntityForTier(nextRow, {
      postType: resolvePostTypeFromBuilderData(props.mainBuilderData),
    });
    if (!tierSafeRow) {
      throw new Error(ENTITY_TIER_IMPORT_BLOCKED_MESSAGE);
    }
    layoutCopy.splice(insertIndex, 0, tierSafeRow);
    setInitialData([...layoutCopy]);
    setIndexes({
      ...indexes,
      type: "row",
  ...
  const onSelectModule = (item) => {
    const layoutPostType = resolvePostTypeFromBuilderData(props.mainBuilderData);
    if (isModuleLocked(item.key, "filter", { postType: layoutPostType })) {
      return;
    }
