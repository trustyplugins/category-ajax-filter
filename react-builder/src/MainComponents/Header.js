...
import {
  mergeFilterModulesIntoLayout,
  resolveImportedFilterMergeSource,
} from "./importExport/mergeImportedFilterLayout";
...
import {
  buildTermNameMapFromTaxonomyList,
  syncTermLabelsInLayoutDocument,
} from "./utils/termLabelSync";
...
function Header(props) {
  ...
  const termLabelSyncLayoutRef = useRef("");
  const mainBuilderDataRef = useRef(mainBuilderData);
  ...
  useEffect(() => {
    mainBuilderDataRef.current = mainBuilderData;
  }, [mainBuilderData]);
  ...
  useEffect(() => {
    ...
    const layoutKey = String(doc?.common_data?.layout_key || "").trim();
    loadGoogleFontFamily(resolveGlobalFontFamilyFromBuilderData(doc));

    // Always mirror parent document — required for Save dirty detection.
    // (Children update MainLayoutContainer state; Header must stay in sync.)
    setMainBuilderData(doc);
    mainBuilderDataRef.current = doc;

    if (!layoutKey || termLabelSyncLayoutRef.current === layoutKey) {
      return undefined;
    }
    termLabelSyncLayoutRef.current = layoutKey;

    let cancelled = false;
    (async () => {
      try {
        const postType = resolvePostTypeFromBuilderData(doc) || "post";
        ...
        const nameMap = buildTermNameMapFromTaxonomyList(
          res.data.taxonomy_list || []
        );
        if (cancelled) {
          return;
        }

        const current = mainBuilderDataRef.current;
        const { doc: synced, changed } = syncTermLabelsInLayoutDocument(
          current,
          nameMap
        );
        ...
        // Refresh labels without marking dirty when the layout is still pristine.
        const wasPristine =
          JSON.stringify(prevMainBuilderData.current) ===
          JSON.stringify(current);
        setMainBuilderData(synced);
        mainBuilderDataRef.current = synced;
        if (wasPristine) {
          prevMainBuilderData.current = cloneDeep(synced);
        }
        props.updatedBuilderData(synced);
      } catch (error) {
        // Leave baked labels if the taxonomy request fails.
      }
    })();
  ...
  const maybeShowPostLayoutGuidance = () => {
    ...
    notification.open({
      title: (
        <div>
          Filter layout is ready. Continue with{" "}
  ...
  const refreshTermLabelsForDocument = async (layoutDoc) => {
    try {
      const postType = resolvePostTypeFromBuilderData(layoutDoc) || "post";
      const res = await apiClient.get(
        apiEndpoints.getTaxonomyRecursiveData(postType)
      );
      ...
      const { doc: synced } = syncTermLabelsInLayoutDocument(
        layoutDoc,
        nameMap
      );
      return synced;
  ...
  const onHandleSaveLayout = async (flag = false) => {
    ...
    try {
      ...
      // Persist live WP term names so the frontend matches renames after save.
      newData = await refreshTermLabelsForDocument(newData);
      const wasDraft =
  ...
  const handleUpdateLayoutTitle = async () => {
    ...
    try {
      ...
      if (response?.data?.status === "success") {
        ...
        message.success("Filter label updated.");
  ...
  const handleImportFromFile = async ({
    ...
    if (scope === "filter_layout") {
      const importedFilterMergeSource = resolveImportedFilterMergeSource(
        importedDoc,
        importedJson
      );
      const shouldMergeFilterModule =
        Boolean(mergeFilterModule) ||
        importedMeta?.scope === "filter_module" ||
        importedFilterMergeSource?.type === "module";

      if (shouldMergeFilterModule) {
        nextBuilder.filter_layout_data = await mergeFilterModulesIntoLayout(
          nextBuilder.filter_layout_data,
          importedFilterMergeSource,
          {
            apiClient,
            apiEndpoints,
            currentPostType,
