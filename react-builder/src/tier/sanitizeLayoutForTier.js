...
const sanitizeFilterLayoutData = (filterLayoutData, options = {}) => {
  if (!filterLayoutData || typeof filterLayoutData !== "object") {
    return filterLayoutData;
  }
  const next = { ...filterLayoutData };
  const postType = options.postType != null ? String(options.postType) : "";
  next.initial_data = filterModulesByTier(next.initial_data, (moduleKey) =>
    canUseFilterModule(moduleKey, { postType })
  );
...
const ENTITY_PRO_ONLY_MESSAGE =
  "This import includes Pro-only modules that are not available in Free. Upgrade to Pro or use a Free-compatible export.";

/**
 * Sanitize a single filter module for Free. Returns null if the module key is Pro-only.
 *
 * @param {object} module Filter module node.
 * @param {{ postType?: string }} [options]
 * @returns {object|null}
 */
export function sanitizeFilterModuleForTier(module, options = {}) {
  if (isProTier() || !module || typeof module !== "object") {
    return module;
  }
  if (module.key && !canUseFilterModule(module.key, options)) {
    return null;
  }
  const next = cloneDeep(module);
  if (next.settings && next.key) {
    next.settings = sanitizeFilterModuleSettings(next.key, next.settings);
  }
...
/**
 * Sanitize filter module / column / row for Free. Drops Pro-only modules.
 * Returns null when nothing usable remains.
 *
 * @param {object} entity Filter entity node.
 * @param {{ postType?: string }} [options]
 * @returns {object|null}
 */
export function sanitizeFilterEntityForTier(entity, options = {}) {
  if (isProTier() || !entity || typeof entity !== "object") {
    return entity;
  }

  const next = cloneDeep(entity);
  const isModule =
    next.type === "module" ||
    (next.key && next.settings && !Array.isArray(next.data));

  if (isModule) {
    return sanitizeFilterModuleForTier(next, options);
  }

  if (next.type === "column" || (Array.isArray(next.data) && next.type !== "row")) {
    next.data = (next.data || [])
      .map((module) => sanitizeFilterModuleForTier(module, options))
      .filter(Boolean);
    if (!canUseFeature("gradient_colors")) {
      flattenGradientsInNode(next);
    }
    return next.data.length > 0 ? next : null;
  }
  ...
  if (next.type === "row") {
    next.data = (next.data || [])
      .map((column) => {
        ...
        return sanitizeFilterEntityForTier(
          { ...column, type: "column" },
          options
        );
      })
    ...
    const hasModules = next.data.some(
      (column) => Array.isArray(column?.data) && column.data.length > 0,
    );
    return hasModules ? next : null;
  }
...
/**
 * Sanitize a single post module for Free entity import/export.
 * Returns null if the module key is Pro-only.
 *
 * @param {object} module Post module node.
 * @returns {object|null}
 */
export function sanitizePostModuleEntityForTier(module) {
  if (isProTier() || !module || typeof module !== "object") {
    return module;
  }
  if (module.key && !canUsePostModule(module.key)) {
    return null;
  }
  const next = cloneDeep(module);
  if (next.settings && next.key) {
...
/**
 * Sanitize post module / column / row for Free. Drops Pro-only modules.
 * Returns null when nothing usable remains.
 *
 * @param {object} entity Post entity node.
 * @returns {object|null}
 */
export function sanitizePostEntityForTier(entity) {
  if (isProTier() || !entity || typeof entity !== "object") {
    return entity;
  }
  ...
  if (isModule) {
    return sanitizePostModuleEntityForTier(next);
  }

  if (next.type === "column" || (Array.isArray(next.data) && next.type !== "row")) {
    next.data = (next.data || [])
      .map((module) => sanitizePostModuleEntityForTier(module))
      .filter(Boolean);
    if (!canUseFeature("gradient_colors")) {
      flattenGradientsInNode(next);
    }
  ...
  if (next.type === "row") {
    next.data = (next.data || [])
      .map((column) => {
        ...
        return sanitizePostEntityForTier({ ...column, type: "column" });
      })
      .filter(Boolean);
    if (!canUseFeature("gradient_colors")) {
      flattenGradientsInNode(next);
...
/**
 * Scrub an entity export payload ({ module_data|column_data|row_data, _export_meta }).
 * Returns null when Free scrubbing removes all usable content.
 *
 * @param {object} payload Entity export payload.
 * @param {"filter"|"post"} area Layout area.
 * @param {{ postType?: string }} [options]
 * @returns {object|null}
 */
export function sanitizeEntityExportPayloadForTier(payload, area = "filter", options = {}) {
  if (isProTier() || !payload || typeof payload !== "object") {
    return payload;
  }

  const sanitize =
    area === "post"
      ? sanitizePostEntityForTier
      : (entity) => sanitizeFilterEntityForTier(entity, options);
  const next = cloneDeep(payload);

  if (next.module_data) {
    next.module_data = sanitize(next.module_data);
    if (!next.module_data) {
      return null;
    }
  }
  if (next.column_data) {
    next.column_data = sanitize(next.column_data);
    if (!next.column_data) {
      return null;
    }
  }
  if (next.row_data) {
    next.row_data = sanitize(next.row_data);
    if (!next.row_data) {
      return null;
    }
  }
...
export const ENTITY_TIER_IMPORT_BLOCKED_MESSAGE = ENTITY_PRO_ONLY_MESSAGE;
...
export function sanitizeLayoutDocumentForTier(layoutDocument) {
  ...
  const layoutPostType =
    next.common_data?.post_type != null
      ? String(next.common_data.post_type)
      : "";
  if (next.filter_layout_data) {
    next.filter_layout_data = sanitizeFilterLayoutData(next.filter_layout_data, {
      postType: layoutPostType,
    });
  }
