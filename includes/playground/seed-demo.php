...
/**
 * Set active theme content width for Playground demos.
 *
 * @param string $content_size CSS length for content width.
 * @param string $wide_size    CSS length for wide alignment (block themes only).
 * @return void
 */
function caf_playground_set_theme_content_width( $content_size = '1280px', $wide_size = '1400px' ) {
	$width_px = (int) preg_replace( '/[^0-9]/', '', $content_size );
	if ( $width_px < 1 ) {
		$width_px = 1280;
	}

	if ( 'hello-elementor' === get_stylesheet() ) {
		$GLOBALS['content_width'] = $width_px;
		add_filter(
			'hello_elementor_content_width',
			static function () use ( $width_px ) {
				return $width_px;
			}
		);
		caf_playground_persist_hello_elementor_content_width( $width_px );
		return;
	}
...
/**
 * Persist Hello Elementor content width for frontend requests after seeding.
 *
 * @param int $width_px Content width in pixels.
 * @return void
 */
function caf_playground_persist_hello_elementor_content_width( $width_px ) {
	$mu_dir = WP_CONTENT_DIR . '/mu-plugins';
	if ( ! is_dir( $mu_dir ) ) {
		wp_mkdir_p( $mu_dir );
	}

	$mu_file = $mu_dir . '/caf-playground-content-width.php';
	...
	file_put_contents(
		$mu_file,
		'<?php' . PHP_EOL
		. '/** Auto-generated by CAF Playground seed. */' . PHP_EOL
		. "add_filter( 'hello_elementor_content_width', static function () { return {$width_px}; } );" . PHP_EOL
	);
}
