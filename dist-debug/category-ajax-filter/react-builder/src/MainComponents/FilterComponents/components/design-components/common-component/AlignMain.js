...
import { normalizeFlexFlowDirection } from "../../settingTabContent/filterDesignTabDerivedState";
...
function syncJustifyContentForRowDirectionChange(previousFlexFlow, nextFlexFlow, styleItem) {
  const rowPair =
    (previousFlexFlow === "row" && nextFlexFlow === "row-reverse") ||
    (previousFlexFlow === "row-reverse" && nextFlexFlow === "row");
...
function AlignMain(props) {
  ...
  const { property, label, styleState = 'default', deviceSwitch, styleTab: originalStyleTab, defaultValue = "", options, isMeta } = props;
  ...
  if (property === "flexFlow") {
    currentalign = normalizeFlexFlowDirection(currentalign);
  }
  ...
  const handleAlign = (e) => {
    ...
    if (type === "module") {
      ...
      if (styleTab !== "") {
        ...
        const previousFlexFlow =
          property === "flexFlow" ? item.flexFlow : undefined;
        item[property] = value;
        if (property === "flexFlow") {
          syncJustifyContentForRowDirectionChange(
            previousFlexFlow,
            value,
            item
          );
