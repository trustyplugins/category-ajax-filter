jQuery(function ($) {
    ...
    const CAFBuilder = {
        ...
        resetRangeSlidersToInitial($root, mode = "initial") {
            const $modules = $root.is(".caf-module-filter.caf-module-type-range_slider")
                ? $root
                : $root.find(".caf-module-filter.caf-module-type-range_slider");
        ...
        resolveSmartTermMatches($builder, keyword) {
            ...
            if (queryNumericMeta.numbers.length > 0) {
                $builder.find(".caf-module-filter.caf-module-type-range_slider").each((_, moduleEl) => {
            ...
            if (CAFUrlState.isEnabled($builder)) {
                ...
                if (urlState && Object.keys(urlState).length) {
                    ...
                    this.initRangeSliders($builder.find(".filter-layout-container").length
                        ...
                        : $builder);
                    if (!urlApplied) {
                        this.buildQuery($builder, 1, { skipUrlUpdate: true });
                    }
                }
            }
